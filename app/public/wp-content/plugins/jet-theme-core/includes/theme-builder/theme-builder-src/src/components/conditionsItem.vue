...
export default {
	...
	computed: {
		...
		subGroupOptions: function() {
			return this.rawConditionsData[ this.сondition.group ]['options'];
		},
		subGroupValueOptions: function() {
			...
			if ( subGroupData.options.length ) {
	...
	methods: {
		...
		getRemoteItems: function( $query ) {
			...
			return wp.apiFetch( {
				...
				conditionsData[ this.сondition.group ]['sub-groups'][ this.сondition.subGroup ]['options'] = response;
