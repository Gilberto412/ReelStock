...
export default {
	...
	computed: {
		getTemplateTypeOptions() {
		    ...
		    let templateTypes = window.JetThemeBuilderConfig.templateTypeOptions || [],
		        defaultTypes = [
			        {
				        label: 'All',
				        value: 'all',
		        ...
		        excludeItems = [ 'jet_header', 'jet_footer' ];

			templateTypes = templateTypes.filter( ( type ) => {
				return ! excludeItems.includes( type.value );
			} );

			return [ ...defaultTypes, ...templateTypes ]
		}
	},
	methods: {
