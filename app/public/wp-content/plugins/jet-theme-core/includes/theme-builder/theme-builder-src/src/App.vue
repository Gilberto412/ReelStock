	...
	computed: {
		templateListClasses() {
			return [
				...
				this.$store.state.updatePageTemplatesProgressState ? 'progress-state-alt' : ''
			];
		},
		pageTemplateList() {
		...
		isLoaderVisible() {
			return this.$store.state.updatePageTemplatesProgressState;
		}
	},
	methods: {
		getTeplatePageList() {
...
.jet-theme-builder {
	...
	--products-archive-type-color: #565FAD;
	--single-product-type-color: #7956AD;
	--product-page-type-color: #A456AD;

	& > .jet-theme-builder__spinner-loader {
		position: fixed;
		width: 32px;
		height: 32px;
